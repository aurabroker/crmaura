...
<svelte:head><title>Nowa Umowa Generalna Gwarancji — FRANK67 CRM</title></svelte:head>
...
<div class="max-w-5xl">
	<div class="flex items-center gap-3 mb-6">
		...
		<div>
			<h1 class="text-2xl font-semibold text-slate-900">Nowa Umowa Generalna Gwarancji</h1>
	...
	<div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
		...
		{/if}
		<PolicyForm bind:this={policyForm} policy={{ typ_umowy: 'generalna', ug_podtyp: 'gwarancje' } as any} />
	</div>

	<div class="flex justify-end gap-3 mt-4">
		<button onclick={() => goto('/bonds')} class="px-5 py-2.5 text-sm border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50">
			Anuluj
		</button>
		<button onclick={save} disabled={saving} class="px-6 py-2.5 text-sm bg-violet-600 text-white rounded-lg font-semibold hover:bg-violet-700 disabled:opacity-60">
