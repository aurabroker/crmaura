<script lang="ts">
	...
	async function saveTU() {
		...
		const { error } = await sb.from('crm_insurers').insert([{
			tenant_id: appState.profile!.tenant_id,
			nazwa: fNazwa.trim(), dzial: fDzial,
			ulica: fUlica || null, nip: fNip || null, krs: fKrs || null
		}]);
		saving = false;
		...
		fNazwa = ''; fDzial = 'Majątkowy'; fUlica = ''; fNip = ''; fKrs = '';
	}
...
<div class="grid grid-cols-2 gap-6">
	...
	<div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
		<div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
			...
			<button onclick={() => showModal = true} class="text-xs border border-slate-200 rounded-lg px-3 py-1.5 text-slate-600 hover:bg-slate-50">+ Dodaj TU</button>
		</div>
		<table class="w-full text-left text-sm">
			...
			<tbody>
				...
				{:else}
					<tr><td colspan="3" class="px-5 py-6 text-center text-slate-400">Brak TU</td></tr>
				{/each}
			</tbody>
		</table>
	</div>
...
<Modal title="Nowe Towarzystwo (TU)" open={showModal} onclose={() => { showModal = false; formError = ''; }}>
	{#snippet footer()}
